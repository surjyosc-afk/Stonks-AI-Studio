...
def find_support_resistance(
    ...
    # Cluster nearby levels to avoid noise
...
def _cluster_levels(prices: np.ndarray, n: int, tol: float = 0.01) -> List[float]:
    ...
    # Return the n levels with the most price touches
    touched = sorted(
        centroids,
...
def detect_head_and_shoulders(
    ...
    for j in range(1, len(pivots) - 1):
        ...
        neckline = round((df["Low"].iloc[ls_idx] + df["Low"].iloc[rs_idx]) / 2, 2)
        patterns.append(
            {
                "date": str(dates[rs_idx].date()),
                "pattern": "head_and_shoulders",
                "head_price": round(head, 2),
