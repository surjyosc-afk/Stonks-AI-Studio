...
def run_opportunity_radar(
    ...
    # ── Sentiment ──
    sentiment_score = sentiment_to_signal(sentiment_results) if sentiment_results else 0.0
    ...
    if pattern_data:
        ...
        has_bullish_breakout = any(
            b["type"] == "bullish_breakout" for b in breakouts[-3:]
        )
        has_bullish_reversal = any(
            r["type"] == "bullish_reversal" for r in reversals[-3:]
        )
    ...
    if abs(momentum) > 1.5:
        ...
        active_signals.append(
            f"Strong {direction} price momentum ({momentum:+.1f}%)"
        )
